...
class _MapsState extends State<Maps> {
  ...
  createPolyLine() async {
    await FirebaseFirestore.instance.collection('routes').get().then((QuerySnapshot querySnapshot) {
    ...
    for (var point in firePoints) {
      ...
      for (var i = 0; i < point.length - 1; i++) {
        PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
          ...
          PointLatLng(point[(i + 1).toString()][0], point[(i + 1).toString()][1]),
          ...
          return Obx(() {
            return GoogleMap(
                ...
                onMapCreated: (controller) {
                  ...
                  _googleMapController.moveToCurrentLocation(currentPosition ?? LatLng(24.619718082927363, 73.8548188383453));
                    ...
                    itemBuilder: (context, index) {
                      return Padding(
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () {
                            _googleMapController.moveToCurrentLocation(_googleMapController.markers![index].position);
                            print(_googleMapController.markers![index].infoWindow.snippet.toString());
                          },
                          child: Column(
                            children: [
                              Text(_googleMapController.markers![index].infoWindow.title.toString()),
                              Text('${_googleMapController.markers![index].infoWindow.snippet} km/h'),
                            ],
                          ),
                        ),
                      );
